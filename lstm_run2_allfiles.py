...
for y in y_array:
    ...
    x_train,y_train,x_vali,y_vali = fr_obj.split_dataset(X_data_use,Y_data,hist_window,horizon,train_split)
    ...
    lstm_model = fr_obj.model_config_lstm1(x_train,y_train,32,8,0.0,0.025,'adadelta','mse')
    ...
    history = lstm_model.fit(x_train,y_train,epochs=500,validation_split=0.15,verbose=1,callbacks=callbacks,shuffle=False)
    ...
    data_val = X_data_use[-n_future:,:]

    val_rescaled = data_val.reshape(data_val.shape[0],1, data_val.shape[1])
    pred = lstm_model.predict(val_rescaled)
    pred_Inverse = Y_scaler.inverse_transform(pred)
    y_exp = Y_scaler.inverse_transform(np.reshape(X_data[-n_future:,counter],(len(X_data[-n_future:,counter]),1)))
    
    X_data_use = np.append(X_data_use,np.reshape(X_data[:,counter],(len(X_data[:,counter]),1)),axis=1)
    print(len(X_data_use))
    X_data_use[-n_future:,counter] = np.reshape(pred_Inverse,len(pred_Inverse))
    ...
    metrics_all = fr_obj.timeseries_evaluation_metrics_func(y_exp,pred_Inverse)
    plt.figure(figsize=(16,9))
    plt.plot(list(y_exp))
